...
Ltac prove_equal :=
  ...
  | |- ?a = ?b =>
    ...
    apply R_add_assoc_general; auto
  end.
...
Lemma R_add_comm_general : forall e1 e2,
  (forall n, count_occ Req_dec_T (elements e1) n = count_occ Req_dec_T (elements e2) n) -> eval_add_expr e1 = eval_add_expr e2.
Proof.
...
Ltac prove_count_occ :=
  match goal with
  | [ |- context [count_occ Req_dec_T ?l1 ?n = count_occ Req_dec_T ?l2 ?n] ] =>
    let rec prove_count_occ_aux l n :=
      match l with
      | ?a :: ?l' =>
        destruct (Req_dec_T a n); prove_count_occ_aux l' n
      | [] => reflexivity
      end in
...
Lemma apsdofjiprimeee : forall n a b c d e,
  count_occ Req_dec_T [a;b;c;d+e] n = count_occ Req_dec_T [d+e;a;c;b] n.
Proof.
  intros. prove_count_occ_2 [a;b;c;d+e] n.
Qed.

Lemma apsdofjiprime : forall a b c d e f g h i n,
  count_occ Req_dec_T [a;b;c;d;e;f;g;h + i] n = count_occ Req_dec_T [a;c;b;d;e;f;g;h+i] n.
Proof.
  intros. prove_count_occ_2 [a;b;c;d;e;f;g;h+i] n.
Qed.

Ltac prove_equal_R_add :=
  let rec to_add_expr e :=
    match e with
    | ?a + ?b =>
      let a' := to_add_expr a in
  ...
  | |- ?a = ?b =>
    ...
    set (l1 := elements e1);
    set (l2 := elements e2);
    assert (forall n, count_occ Req_dec_T (l1) n = count_occ Req_dec_T (l2) n) as H1
    by (intros __n__; simpl in l1, l2; unfold l1, l2; prove_count_occ); apply R_add_comm_general in H1; auto
  end.

Lemma prove_equal_R_add_example : forall a b c d,
  ((d + d) + a) + ((b + c) + (c + c)) = (c + b) + (c + a) + (c + d) + d.
Proof.
  intros. prove_equal_R_add.
Qed.
